"""A module that loads a list of words from an online dictionary, filters
out the words that can be used to create palingrams, and saves a list of
palingrams to a file."""
...
def compute_palingrams(words_list : List[str]) -> List[str]:
    ...
    for word in tqdm(words_list):
        for idx in range(len(word) - 1):
            ...
            if is_palindrom(word_first_part) and word_second_part[::-1] in words_list:
                word_to_add = word_second_part[::-1] + ' ' + word

            elif word_first_part[::-1] in words_list and is_palindrom(word_second_part):
...
def main() -> None :
    ...
    palingrams_list = compute_palingrams(words_list)
